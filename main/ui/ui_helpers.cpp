...
{
    lv_obj_flag_t f = (lv_obj_flag_t)flag;
    if(value == _UI_MODIFY_FLAG_TOGGLE) {
        if(lv_obj_has_flag(target, f)) lv_obj_remove_flag(target, f);
        else lv_obj_add_flag(target, f);
    }
    else if(value == _UI_MODIFY_FLAG_ADD) lv_obj_add_flag(target, f);
    else lv_obj_remove_flag(target, f);
}
void _ui_state_modify(lv_obj_t * target, int32_t state, int value)
{
    lv_state_t s = (lv_state_t)state;
    if(value == _UI_MODIFY_STATE_TOGGLE) {
        if(lv_obj_has_state(target, s)) lv_obj_remove_state(target, s);
        else lv_obj_add_state(target, s);
    }
    else if(value == _UI_MODIFY_STATE_ADD) lv_obj_add_state(target, s);
    else lv_obj_remove_state(target, s);
}
