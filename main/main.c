...
QueueHandle_t time_update_queue;
                                ...
                                int32_t event_id, void* event_data)
{
    if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_START) {
        ...
        init_ntp_client(time_update_queue);
        s_retry_num = 0;
...
void wifi_init_sta(void)
{
    ...
    if (bits & WIFI_CONNECTED_BIT) {
        ...
        // Try to manually re-sync now that we're connected
        // ESP_LOGI(TAG, "Attempting to sync NTP server");
        // if (ntp_client_sync(10, 1000) == 1) {
        //    ntp_client_get_time();
        //}
...
// Pin number 2 is used for back light on the 5" model
...
void app_main(void)
{
    ...
    ESP_LOGI(TAG, "Setting software 180 degree software rotation");
    ...
    // this originally didn't use malloc
    // however we need to dynamically allocate memory here for the touch panel
